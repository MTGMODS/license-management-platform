...
def activate_key(payload: ActivateKeyDTO, db: Session = Depends(get_db)):
    ...
    license_id = service.activate_key_for_user(payload.key, payload.user_id)
...
async def request_download(payload: DownloadRequestDTO, db: Session = Depends(get_db)):
    ...
    await publish_file_generation_event(user_id=payload.user_id, media_id=payload.media_id, expire_date=expire_date)
